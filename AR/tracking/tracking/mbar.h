...
private:	
	...
	void find_contours();
	void draw_contours(cv::Mat & img, std::vector<cv::Point2f>& contours, cv::Scalar color);
	void draw_contours(cv::Mat & img, std::vector<cv::Point>& contours, cv::Scalar color);
	void crop_markers();
	void read_markers();	
	void refine_corners();
	void solve_pnp();
